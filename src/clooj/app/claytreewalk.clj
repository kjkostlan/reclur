...
(defn vis-walk [tree f]
 "walks through all visible stuff recursivly. :children-visible? decides whether we go recursivly.
  ...
  (throw (Exception. "Clwalk need root node"))
...
(defn _assign-paths [tree path vis-only?]
  (throw (Exception. "Useless function now"))
...
(defn vis-tree-unwrap [tree]
  ...
  (throw (Exception. "clwalk useless (replace with get vis nodes)"))
...
(defn common-root [paths]
  ...
  (throw (Exception. "Heavy modification needed: nodes instead of paths."))
  (if (= (count paths) 0) []
