; Keep this around for now, don't know if useful.
(ns clooj.safe_mode
  (:import (javax.swing UIManager JFrame SwingUtilities) 
    ...
    (:require [clooj.java.gui :as gui] [clooj.java.file :as file]
...
(defn file2folder [^String file]
  ...
  (let [match (last (file/re-index (file/str-to-regex (sep)) file))]
    (if (nil? match) 
      file ; no change.
      (subs file 0 match))))
