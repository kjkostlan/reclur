...
(defn make-settings-window!! [apply-fn]
  (let [bounds (.getBounds @app_old/frame)
    ...
    (doto button-pane
      ...
      (.add (utils/create-button "Apply" #(apply-fn @settings)))
...
(defn show-settings-window!! [apply-fn]
  (reset! settings @app_old/settings)
  (.show (make-settings-window!! apply-fn)))
