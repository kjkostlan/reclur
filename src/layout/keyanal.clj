...
(defn normal? [kevt]
  "There may be modifiers on in the background, but this particular keypress would be a key that types a character."
  (and (not (arrow-key kevt))
    (or
     (.contains "`1234567890-=\tqwertyuiop[]\\asdfghjkl;'\nzxcvbnm,./ ~!@#$%^&*()_+QWERTYUIOP{}|ASDFGHJKL:\"ZXCVBNM<>?"
       ^String (str (char (:KeyCode kevt))))
      (= (:KeyCode kevt) 192) (= (:KeyCode kevt) 222))))

(defn lowercase-char [kevt] "Ctrl+s => the string 's'"
  (let [kc (:KeyCode kevt)]
    (cond (= kc 192) "`" (= kc 222) "'" (arrow-key kevt) nil
      :else (.toLowerCase ^String (str (char (:KeyCode kevt)))))))
(defn match-char? [kevt kchar] "Normal key events."
  (= (.toLowerCase ^String (str kchar)) (lowercase-char kevt)))
...
(defn match-letter-or-phrase? [kevt txt]
  (let [txtl (.toLowerCase ^String txt)]
    (or (match-char? kevt txtl) (match-arrow? kevt txtl)
