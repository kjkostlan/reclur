...
(ns coder.lang.clojure
 (:require [clojure.string :as string] [clojure.pprint :as pprint]
   [coder.rcode :as rcode] [collections :as collections]
   [coder.rcodeprotocol :as rcodeprotocol]))
...
(defn syntax-resolve [x nms]
  ...
  (if nms
    (cond (and (coll? x) (let [x0 (first x)] (and (not= x0 `unquote) (not= x0 `unquote-splicing) (not= x0 'clojure.core/unquote-conditional) (not= x0 'clojure.core/unquote-conditional-splicing)))) 
      ...
      (symbol? x) (rcode/tmet x (if-let [y (var2sym (ns-resolve nms x))] y x) :pre-ns-resolve x)
      :else x) x))
...
(def tok-matchers 
  [^:picky [3 "false"] ^:picky [3 "true"] ^:picky [3 "nil"] ; vary few symbol-like literals.
   ...
   [1 "+" "e:0._'+/?#'"] [1 "?" "e:0._'+/?#'"] [1 "/" "e:0._'+/?#'"]]) ;kwds + syms, syms lowest priority
...
(deftype ClojureLang [] rcodeprotocol/TwoWayParser
...
;(println "Clojure has been loaded!!!!!!")